...
def schedule_analysis_stock():
  ...
  if currentTime >= nextTimeStamp:
    try:
      ...
      write_next_time_stamp((nextTimeStamp).strftime('%Y-%m-%d %H:%M:%S'))
...
if __name__ == "__main__":
  ...
  stock_data = get_historical_data("LUNA/USDT", TIME_INTERVAL_STR)
  invertHammer = abstract.CDLADVANCEBLOCK(stock_data)
  print(invertHammer[invertHammer != 0])
  print(stock_data.iloc[332]["timestamp"])

  # real = abstract.HT_TRENDLINE(stock_data["close"])

  # print(real[250])

  # df = investpy.get_stock_historical_data(stock='BSR', country='VietNam', from_date='01/01/2021', to_date='08/04/2022')
  # df.drop('Currency', axis=1, inplace=True)

  # df.rename(columns={'name': 'timestamp', 'Open': 'open', 'High': 'high', 'Low': 'low', 'Close': 'close', 'Volume': 'volume'}, inplace=True)
  ...
  # stockdata = []
  # print(not stockdata)
  # print(any("a" in s for s in stockdata))
